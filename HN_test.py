...
for i in range(1,int(init_pop_size)+1):
	...
	run_hyperneat = subprocess.Popen(["python", "Simple_test.py"],cwd="/home/jharvard/Pop_files/Scripts/",stdout=subprocess.PIPE)  #TODO Adjust for wherever HNeat is
	run_hyperneat.wait()
	...
	if (run_hyperneat.returncode != 0):
		...
		with io.open(path+"/"+str(RobotID).zfill(5)+".xml",mode='w') as f:
			f.write(run_hyperneat.stdout.readlines())
			io.close()
			RobotID += 1
